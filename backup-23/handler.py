...
def ask_groq(prompt: str) -> str:
    ...
    try:
        ...
        if e.code == 429:
            ...
            return "Groq AI returned an error. Please try again later."
    except Exception as e:
        print("Error calling Groq:", repr(e))
        return "Sorry, I could not get a Groq AI reply right now."
...
def handle_text_message(chat_id: int, message_id: int, text: str, first_name: str | None):
    ...
    if text == "/groq":
     ...
     # This should be BEFORE the fallback but AFTER all command checks
    if not text.startswith("/"):
        # Save the message to DynamoDB (already done at the end)
        # But also respond with AI if Groq is configured
        if GROQ_API_KEY:
            reply = ask_groq(text)
            send_message(chat_id, reply)
            return
